...
public class SaveCartModuleTest extends PageFactoryInitializer {
	...
	public void saveCartDeleteAndVerify(String saveCartName) throws Exception {
		...
		homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink().saveCartPage()
				.clickOnTheCreatedSaveCart(saveCartName).deleteSaveCart()
				.verifyAlertText(data.getDeleteSaveCartAlertText()).homePage().clickOnUserAccountDropdown()
				.clickOnMySaveCartLink().saveCartPage().verifyDeletionOfSaveCart(saveCartName);
	}
	...
	public void TC_ShoppingCart_011_CreateAndDeleteSaveCart() throws Exception {
		...
		try {
			...
			String productName = homePage().searchText(searchText).clickOnSearch().productDetailsPage()
					.getProductName();
			productDetailsPage().clickOnAddToCartButton().myCartPage().clickOnCheckoutInMyCartPopup()
					.verifyNameOfTheProductInMyCartPage(productName).myCartPage().clickOnSaveCartButton()
			...
			homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink().saveCartPage()
					.verifybreadCrumbs(saveCartBreadcrumb).verifyPageName(saveCartBreadcrumb).verifySaveCartTitle()
					.clickOnTheCreatedSaveCart(saveCartName).verifybreadCrumbs(saveCartName)
					.verifyPageName(saveCartName).verifyTitleAfterClickingOnTheCartCreated();
	...
	@TestCaseId("{0}")
	@Test(groups = { "SaveCartModule",
			"regression" }, dataProvider = "mutipleSheetsSingleWorkbook", dataProviderClass = DataDrivenTestingFromExcel.class)
	public void sC_BulkAlert_ItemNotChosen(String testcaseId, @Parameter("Bulk Option") String bulkOption,
			@Parameter("Expected Alert Text") String expectedAlertText) throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					.clickOnCheckoutInMyCartPopup().myCartPage().clickOnSaveCartButton()
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation();
			Thread.sleep(1000);
			myCartPage().verifySaveCartCreationMessage(saveCartName).homePage().clickOnUserAccountDropdown()
					.clickOnMySaveCartLink().saveCartPage().clickOnTheCreatedSaveCart(saveCartName)
					.selectBulkActionsDropdown(bulkOption).verifyAlertText(expectedAlertText);
	...
	public void verifyBreadCrumbFromMyAccount() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					.clickOnCheckoutInMyCartPopup().clickOnSaveCartButton()
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().clickOnUserAccountDropdown().clickOnMyAccount().myAccountsPage().clickOnGroupsTab()
					.saveCartPage().clickOnTheCreatedSaveCart(saveCartName).clickOnMySavedCartCrumb()
					.verifyPageName(saveCartBreadCrumb).clickOnMyAccountCrumb().myAccountsPage()
	...
	public void verifySavedCartPageContent() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					.clickOnCheckoutInMyCartPopup().clickOnSaveCartButton()
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink()
					.clickOnTheCreatedSaveCart(saveCartName).saveCartPage().verifyMySavedCartPage();
	...
	public void verifiesNewSavedCart() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					...
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink()
					.clickOnTheCreatedSaveCart(saveCartName).saveCartPage().verifyPageName(saveCartName);
		} catch (UnhandledAlertException e) {
			TestUtility.alertAccept();
			throw new Exception("Alert not handled.");
		} finally {
	...
	public void verifiesAddingItemsToExistingCart() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().homePage()
					.searchText(searchAnotherText).clickOnSearch().productDetailsPage().clickOnAddToCartButton()

					.myCartPage().clickOnCheckoutInMyCartPopup().clickOnSaveCartButton()
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink()
					.clickOnTheCreatedSaveCart(saveCartName).saveCartPage().verifyPageName(saveCartName);
	...
	public void verifiesSavedCartListFromDifferentPages() throws Exception {
		...
		try {
			...
			homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink().saveCartPage().verifySavedCartList()
					.homePage().clickOnUserAccountDropdown().clickOnMyAccount().myAccountsPage().clickOnGroupsTab()
					.clickOnViewMore().verifyPageName().homePage().searchText(searchText).clickOnSearch()
					.productDetailsPage().clickOnAddToCartButton().myCartPage().clickOnCheckoutInMyCartPopup()
					.clickOnSaveCartButton().saveCartPage();
	...
	public void verifyBulkOptionDeleteSelectedItems() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					...
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink();
			saveCartPage().clickOnTheCreatedSaveCart(saveCartName).clickOnFirstSelectCheckbox().saveCartPage()
	...
	public void verifiesBulkOptionUpdateWhenQuantityIsZero() throws Exception {
		...
		try {
			...
			productDetailsPage().clickOnAddToCartButton().myCartPage().clickOnCheckoutInMyCartPopup()
					.clickOnSaveCartButton().enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName)
					.hitEnterForSaveCartCreation().homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink()
					.saveCartPage().clickOnTheCreatedSaveCart(saveCartName).enterQuantity("0")
					.clickOnFirstSelectCheckbox().verifyAlertText("Min Order Quantity is 1. For PN: " + partNumber
	...
	public void verifiesBulkOptionUpdateWhenQuantityIsNull() throws Exception {
		...
		try {
			...
			productDetailsPage().clickOnAddToCartButton().myCartPage().clickOnCheckoutInMyCartPopup()
					.clickOnSaveCartButton().enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName)
					.hitEnterForSaveCartCreation().homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink()
					.saveCartPage().clickOnTheCreatedSaveCart(saveCartName).enterQuantity("")
					.clickOnFirstSelectCheckbox().verifyAlertText("Invalid Qty. For PN: " + partNumber
							+ ". To Continue with Min Order Qty click \"Ok\".To cancel this item click \"Cancel\"");
		} catch (UnhandledAlertException e) {
			TestUtility.alertAccept();
			throw new Exception("Alert not handled.");
	...
	public void verifiesBulkOptionAddingOneItemToCartWithSelectOption() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					...
					.homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink()
					.clickOnTheCreatedSaveCart(saveCartName).saveCartPage().verifyPageName(saveCartName)
					.clickOnFirstSelectCheckbox().selectBulkActionsDropdown(data.getBulkOption())
					.verifyNumberAddedSuccessfullyMsg(1).myCartPage().clickOnCloseButtonInMyCartPopUp();
	...
	public void verifyDeletingItemsFromSavedCartWithSelectAllOption() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					.clickOnCheckoutInMyCartPopup().clickOnSaveCartButton()
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().searchText(searchAnotherText).clickOnSearch().productListPage().clickOnSpecificItem(1)
					.productDetailsPage().clickOnAddToCartButton().myCartPage().clickOnCheckoutInMyCartPopup()
					.clickOnSaveCartButton().enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName)
					.hitEnterForSaveCartCreation().homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink()
					.clickOnTheCreatedSaveCart(saveCartName).saveCartPage().verifyPageName(saveCartName)
					.clickOnSelectAllCheckBox().selectBulkActionsDropdown(data.getBulkOption())
	...
	public void verifyChangeOfExtensionPrice() throws Exception {
		...
		try {
			...
			myCartPage().enterQuantityInShoppingCart(data.getQuantityForShoppingCartPageVerification())
					.myProductGroupsPage().clickOnTheSpecificCheckbox(1).selectBulkActionsDropdown(data.getBulkOption())
					.myCartPage()
	...
	public void verifiesAddingItemsToCartWithSelectAllOption() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					.clickOnCheckoutInMyCartPopup().clickOnSaveCartButton()
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().searchText(searchAnotherText).clickOnSearch().productDetailsPage()
					.clickOnAddToCartButton().myCartPage().clickOnCheckoutInMyCartPopup().clickOnSaveCartButton()
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink()
					.clickOnTheCreatedSaveCart(saveCartName).saveCartPage().verifyPageName(saveCartName)
					.clickOnSelectAllCheckBox().selectBulkActionsDropdown(data.getBulkOption())
					.verifyNumberAddedSuccessfullyMsg(2).myCartPage().clickOnCloseButtonInMyCartPopUp();
	...
	@Test(groups = { "SaveCartModule",
			"regression" }, dataProvider = "mutipleSheetsSingleWorkbook", dataProviderClass = DataDrivenTestingFromExcel.class)
	public void verifyEditCartName_ES_PS(String testcaseId, @Parameter("Save Cart Name") String saveCartName,
			@Parameter("Save Cart Name") String editCartName,
			@Parameter("Expected Alert Text") String expectedAlertText) throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					...
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink().saveCartPage()
					.clickOnTheCreatedSaveCart(saveCartName).clickOnEditCartName().verifyEditCartNameElements()
					.editCartNameTextBox(editCartName).clickOnSaveIconOfEditCartName()
	...
	public void verifyEditCartNameFunctionalityWithExistingCartName() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					.clickOnCheckoutInMyCartPopup().clickOnSaveCartButton()
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton()
					.myCartPage().clickOnCheckoutInMyCartPopup().clickOnSaveCartButton()
					.enterNameOfSaveCartAndAddTheProductToSaveCart(editSaveCartName).hitEnterForSaveCartCreation()
					.homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink().saveCartPage()
					.clickOnTheCreatedSaveCart(saveCartName).clickOnEditCartName().editCartNameTextBox(editSaveCartName)
					.clickOnSaveIconOfEditCartName().verifyAlertText(data.getAlertTextCartNameAlreadyExists());
		} catch (UnhandledAlertException e) {
			TestUtility.alertAccept();
			throw new Exception("Alert not handled.");
		} finally {
	...
	public void verifyCancelFunctionality() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					...
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink().saveCartPage()
					.clickOnTheCreatedSaveCart(saveCartName).clickOnEditCartName().clickOnCancelIconOfEditCartName()
	...
	public void verifyDeleteCartFunctionality() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					...
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink().saveCartPage()
					.clickOnTheCreatedSaveCart(saveCartName).clickOnDeleteCart();
	...
	public void verifySearchFunctionalityInMySavedCart() throws Exception {
		...
		try {
			...
			homePage().searchText(searchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton().myCartPage()
					.clickOnCheckoutInMyCartPopup().clickOnSaveCartButton()
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation();
			homePage().searchText(anotherSearchText).clickOnSearch().productDetailsPage().clickOnAddToCartButton()
					.myCartPage().clickOnCheckoutInMyCartPopup().clickOnSaveCartButton()
					.enterNameOfSaveCartAndAddTheProductToSaveCart(saveCartName).hitEnterForSaveCartCreation()
					.homePage().clickOnUserAccountDropdown().clickOnMySaveCartLink().saveCartPage()
					.clickOnTheCreatedSaveCart(saveCartName).searchTextBoxInSavedCart(searchText).clickOnGoButton()
					.verifyItemDisplay(searchText).searchTextBoxInSavedCart(anotherSearchText).clickOnGoButton()
					.verifyItemDisplay(anotherSearchText).clickOnClearSearchButton()
					.searchTextBoxInSavedCart(invalidText).clickOnGoButton().verifyNoResultsFoundMsg()
					.clickOnClearSearchButton().verifyItemDisplay(searchText).verifyItemDisplay(anotherSearchText);
		}
